...
public class GuiHandler
{
    ...
    public static Vec3 applyGUIModelView(RenderPass currentPass, PoseStack pMatrixStack)
    {
            ...
            if (mc.level != null && (mc.screen == null || !dh.vrSettings.floatInventory))
            {
                ...
                if (!dh.vrSettings.seated && dh.vrSettings.vrHudLockMode != VRSettings.HUDLock.HEAD)
                {
                    ...
                    else if (dh.vrSettings.vrHudLockMode == VRSettings.HUDLock.WRIST)
                    {
                        ...
                        scale = 0.4F;
                        guilocal = new Vec3((double)((float)i * -0.136F * dh.vrPlayer.vrdata_world_render.worldScale), (flag ? 0.13D : 0.12D) * (double)mc.vrPlayer.vrdata_world_render.worldScale, 0.06D * (double)mc.vrPlayer.vrdata_world_render.worldScale);
        ...
        if ((dh.vrSettings.seated || dh.vrSettings.menuAlwaysFollowFace) && mc.gameRenderer.isInMenuRoom())
        {
            scale = 2.0F;
            ...
            float f4 = mc.gameRenderer.isInMenuRoom() ? 2.5F * dh.vrPlayer.vrdata_world_render.worldScale : dh.vrSettings.hudDistance;

...
public class MenuWorldDownloader {
    ...
    public static void downloadWorld(String path) throws IOException, NoSuchAlgorithmException {
        ...
        if (file.exists()) {
            ...
            if (localSha1.equals(remoteSha1)) {
                org.vivecraft.common.utils.Utils.logger.info("MenuWorlds: SHA-1 matches for {}", path);
                return;
            }
        }
        org.vivecraft.common.utils.Utils.logger.info("MenuWorlds: Downloading world {}", path);
    ...
    private static InputStream getStreamForWorld(MenuWorldItem world) throws IOException, NoSuchAlgorithmException {
        if (world.file != null) {
            org.vivecraft.common.utils.Utils.logger.info("MenuWorlds: Using world {}", world.file.getName());
            return new FileInputStream(world.file);
        } else if (world.path != null) {
            downloadWorld(world.path);
            org.vivecraft.common.utils.Utils.logger.info("MenuWorlds: Using official world {}", world.path);
    ...
    private static InputStream getRandomWorldFallback() throws IOException, NoSuchAlgorithmException {
        org.vivecraft.common.utils.Utils.logger.info("MenuWorlds: Couldn't find a world, trying random file from directory");

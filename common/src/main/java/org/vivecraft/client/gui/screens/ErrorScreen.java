...
public class ErrorScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new Builder(Component.translatable("gui.back"), (p) ->
            Minecraft.getInstance().setScreen(this.lastScreen)).pos(this.width / 2 + 5, this.height - 32).size(150, 20).build()
        );
        this.addRenderableWidget(new Builder(Component.translatable("chat.copy"), (p) ->
            Minecraft.getInstance().keyboardHandler.setClipboard(error.getString())).pos(this.width / 2 - 155, this.height - 32)
            .size(150, 20)
            .build()
        );
    }

...
public class ServerConfig {
    ...
    public static BooleanValue pvpNotifyBlockedDamage;

    public static BooleanValue climbeyEnabled;
    public static InListValue<String> climbeyBlockmode;
    public static ListValue<String> climbeyBlocklist;
        ...
        if (listener == null) {
            listener = (action, path, incorrectValue, correctedValue) -> {
                if (incorrectValue != null) {
                    logger.info("Corrected " + String.join(".", path) + ": was " + incorrectValue + ", is now " + correctedValue);
                }
            };
        }

...
import org.vivecraft.client_vr.render.VRCamera;
...
//		}
	...
	public void DrawScopeFB(PoseStack matrixStackIn, int i) {
		if (ClientDataHolder.getInstance().currentPass != RenderPass.SCOPEL && ClientDataHolder.getInstance().currentPass != RenderPass.SCOPER) {
			...
			int light = LevelRenderer.getLightColor(this.minecraft.level, BlockPos.containing(ClientDataHolder.getInstance().vrPlayer.vrdata_world_render.getController(i).getPosition()));

...
public class VRData
{
    ...
    public VRData(Vec3 origin, float walkMul, float worldScale, float rotation)
    {
        ...
        {
            VRSettings vrsettings = DataHolder.getInstance().vrSettings;
    ...
    public float getFacingYaw()
    {
        ...
        {
            Vec3 vec3 = this.c1.getPosition().subtract(this.c0.getPosition()).normalize().yRot((-(float)Math.PI / 2F));
            return DataHolder.getInstance().vrSettings.reverseHands ? (float)Math.toDegrees(Math.atan2(vec3.x, -vec3.z)) : (float)Math.toDegrees(Math.atan2(-vec3.x, vec3.z));
        }
    }

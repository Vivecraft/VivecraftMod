 ...
 */
public record CrawlPayloadS2C(boolean allowed, NetworkVersion targetNetworkVersion) implements VivecraftPayloadS2C {
    ...
    public static CrawlPayloadS2C read(FriendlyByteBuf buffer) {
        if (buffer.readableBytes() > 0) {
            return new CrawlPayloadS2C(buffer.readBoolean(), NetworkVersion.OPTION_TOGGLE);
        } else {
            // old servers always allowed it when they sent this packet
            return new CrawlPayloadS2C(true, NetworkVersion.LEGACY);
        }
    }
}
